...
void add(int x, int amount) {
    for (int i = x; i <= 32001; i += lowbit(i)) { // Substitute max array index for 3200
        bit[i] += amount;
    }
}
...
#include <algorithm>
vector<int> v;
make_heap(v.begin(), v.end(), greater<int>()); // Build heap
x = v.front();pop_heap(v.begin(), v.end(), greater<int>()); v.pop_back(); // Pop heap
v.push_back(x); push_heap(v.begin(), v.end(), greater<int>()); // Insert element
