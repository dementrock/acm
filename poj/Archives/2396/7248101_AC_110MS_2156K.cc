...
struct Edge{
	int v,w,initw;
	Edge *next,*pair;
} *edge[MAXN],mempool[MAXE],tmempool[MAXE],*head;
...
int sap(){
//	memcpy(tmempool,mempool,sizeof(mempool));
	...
	for(Edge *tmp=edge[S];tmp;tmp=tmp->next){
		if(tmp->initw&&tmp->w){
	//		memcpy(mempool,tmempool,sizeof(tmempool));
			return 0;
		}
	}
//	memcpy(mempool,tmempool,sizeof(tmempool));
...
/*	unsigned int Left=0,Right=oo-1,Mid;
