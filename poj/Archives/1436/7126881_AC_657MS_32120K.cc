...
int n,c[8*MAXN],tot,vis[MAXN];
int map[MAXN][1001];
void printtree(int p, int left, int right)
{
	printf("%d %d %d\n",c[p],left,right);
	if(left==right) return;
	int mid=left+right>>1;
	printtree(p*2,left,mid);
	printtree(p*2+1,mid+1,right);
}
...
//	printf("%d %d %d %d %d %d\n",p,left,right,y1,y2,color);
	...
	if(y2<=mid)
		...
		insert(p*2+1,mid+1,right,mid+1,y2,color);
	}
}
...
//		c[p]=0;
	...
	if(y2<=mid)
		...
		dealwith(p*2+1,mid+1,right,mid+1,y2,id);
	}
}
void print()
{
