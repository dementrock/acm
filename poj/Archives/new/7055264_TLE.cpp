...
int maxdep,sortarr[21][100001],n,m,a[100001],l,r,loc[100001],init[100001];
...
{
	if(l<=left&&right<=r) return bifind(left,right,depth,k);
	int ans=0;
	int mid=left+right>>1;
	if(mid>=l) ans+=work(left,mid,depth+1,k);
	if(r>=mid+1) ans+=work(mid+1,right,depth+1,k);
	return ans;
}
