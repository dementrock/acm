...
#include<string.h>
#define MAXN 2000001
int ta[MAXN],a[MAXN],n,id[MAXN],counter[MAXN],maxnum,h[MAXN],size,ans=999999999;
...
void heapdown(int now)
{
	...
	while(1)
	{
		...
		if(heapmax==t) return;
		swap(h[heapmax],h[t]);
		swap(id[counter[heapmax]],id[counter[t]]);
		swap(counter[heapmax],counter[t]);
		t=heapmax;
	}
}
void heapup(int now)
{
	...
	{
		swap(h[t],h[t>>1]);
		swap(id[counter[t]],id[counter[t>>1]]);
		swap(counter[t],counter[t>>1]);
		t=t>>1;
	}
}
...
void work()
{
	...
	//	printf("%d %d\n",a[i],i);
	//	for(int j=1;j<=maxnum;++j) printf("%d ",h[j]);
	//	printf("\n");
