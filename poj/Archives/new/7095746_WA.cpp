...
#include<stdlib.h>
#include<string.h>
int n,f[101][3],edge[101][101],map[101][101],vis[101],oo=99999999;
	...
	if(k==0)
	{
		for(int i=1;i<=edge[x][0];++i)
			for(int j=1;j<=edge[x][0];++j) if(i!=j)
			{
				...
				f[x][k]=min(f[x][k],sum-f[edge[x][i]][0]-f[edge[x][j]][0]+min(f[edge[x][i]][1],f[edge[x][i]][2])+min(f[edge[x][j]][1],f[edge[x][j]][2])+1);
			}
		for(int i=1;i<=edge[x][0];++i)
		{
			if(f[edge[x][i]][2]==-1) dp(edge[x][i],2);
