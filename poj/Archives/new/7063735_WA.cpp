...
#include<stdlib.h>
#include<string.h>
unsigned char f[2][60000],n,m,k,t,map1[160][20],map2[160][20],now;
void dfs(int x, int s1, int s2, int num)
{
	if(x>m+1) return;
	if(x==m+1){f[now&1][s1]=f[now&1][s1]<f[1-now&1][s2]+num?f[1-now&1][s2]+num:f[now&1][s1];return;}
