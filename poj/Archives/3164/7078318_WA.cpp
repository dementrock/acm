				...
				if(flagg)			//若有环，将i作为收缩后点的编号 
				{
					...
					for(int j=1;j<=list[0];++j)
					{
						...
						for(int k=1;k<=n;++k) if(!cut[k])
						{
							...
							{
							//	printf("%d %d %d %.2f\n",now,k,list[1],in);
								if(nowv[now][k]<oo)	//若为出边 
									if(tv[list[1]][k]>nowv[now][k])
										tv[list[1]][k]=nowv[now][k];
								if(nowv[k][now]<oo) //若为入边
									if(tv[k][list[1]]>nowv[now][k]-in)
...
int main()
{
	while(scanf("%d%d",&n,&m)!=EOF)
	{
		...
		int i,j;
		for(int i=1;i<=n;++i) for(int j=1;j<=n;++j) v[i][j]=oo;
		for(int k=1;k<=m;++k)
		{
			scanf("%d%d",&i,&j);
			if(i==j||j==1) continue;
