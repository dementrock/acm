...
long long mult[30][201][201],v[201][201],ans[201][201],matrix[201][201],oo;
	...
	for(int p=2;p<=t;++p)
	{
		for(int k=1;k<=n;++k)
			for(int i=1;i<=n;++i) //if(i!=k)
				...
				if(mult[p-1][i][k]<oo&&mult[p-1][k][j]<oo)
	...
	for(int ii=1;ii<=t;++ii) if(binary[ii])
	{
		...
		for(int p=ii+1;p<=t;++p) if(binary[p])
		{
			...
			for(int k=1;k<=n;++k)
				for(int i=1;i<=n;++i)// if(i!=k)
					for(int j=1;j<=n;++j)// if(j!=k&&i!=j)
					if(matrix[i][k]<oo&&mult[p][k][j]<oo)
